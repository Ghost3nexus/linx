export default function Footer() {
  return (
    <footer
      className="border-t border-[#F0F0F0] py-10 px-6 mb-16 md:mb-0"
      style={{ paddingBottom: "max(env(safe-area-inset-bottom), 2.5rem)" }}
    >
      <div className="max-w-[1200px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-6">
        <div className="text-[14px] text-[#999999]">
          <span className="text-[#1A1A1A] font-bold text-[16px]">
            LIN<span className="text-[#06C755]">X</span>
          </span>
          <span className="ml-3">by TomorrowProof</span>
        </div>

        <nav className="flex flex-wrap gap-x-6 gap-y-2 text-[14px] text-[#999999]">
          <a href="/tokushoho" className="hover:text-[#06C755] transition-colors touch-manipulation">
            特定商取引法
          </a>
          <a href="/privacy" className="hover:text-[#06C755] transition-colors touch-manipulation">
            プライバシーポリシー
          </a>
          <a href="/ai-disclaimer" className="hover:text-[#06C755] transition-colors touch-manipulation">
            AI免責事項
          </a>
          <a
            href="https://calendar.app.google/AJXwDSRvDQEWTxjb7"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-[#06C755] transition-colors touch-manipulation"
          >
            お問い合わせ
          </a>
        </nav>

        <p className="text-[13px] text-[#CCCCCC]">
          &copy; 2026 TomorrowProof Inc.
        </p>
      </div>
    </footer>
  );
}
