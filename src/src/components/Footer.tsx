export default function Footer() {
  return (
    <footer className="pt-12 px-6 pb-[100px] md:pb-12">
      <div className="section-divider mb-12" />
      <div className="max-w-[1200px] mx-auto flex flex-col sm:flex-row items-center justify-between gap-8">
        <div className="text-[14px] text-[#999999]">
          <span className="text-[#1A1A1A] font-bold text-[18px]">
            LIN<span className="text-[#06C755]">X</span>
          </span>
          <span className="ml-3">by TomorrowProof</span>
        </div>

        <nav className="flex flex-wrap gap-x-8 gap-y-2 text-[14px] text-[#999999]">
          <a href="/tokushoho" className="hover:text-[#06C755] transition-colors duration-300 touch-manipulation">
            特定商取引法
          </a>
          <a href="/privacy" className="hover:text-[#06C755] transition-colors duration-300 touch-manipulation">
            プライバシーポリシー
          </a>
          <a href="/ai-disclaimer" className="hover:text-[#06C755] transition-colors duration-300 touch-manipulation">
            AI免責事項
          </a>
          <a
            href="https://calendar.app.google/AJXwDSRvDQEWTxjb7"
            target="_blank"
            rel="noopener noreferrer"
            className="hover:text-[#06C755] transition-colors duration-300 touch-manipulation"
          >
            お問い合わせ
          </a>
        </nav>

        <p className="text-[13px] text-[#CCCCCC]">
          &copy; 2026 TomorrowProof Inc.
        </p>
      </div>
    </footer>
  );
}
